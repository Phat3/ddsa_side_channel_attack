...
In this attack the distincton between bit level and byte level has no meaning because the time complexity is always O(1) regardless the type of the fault injected. The private key is cracked in less than one second even if the fault is injected at the word level.

...
During the fault injection we have to consider a completely random fault that range from the best to the worst scenario. For this reason we have taken the average time of 30 attack's round, and not the time of a single . The following is a better explanation about the meaning of the best and worst scenario.
...
\\
These are the times necessary in order to complete the attack,for each key length:


\begin{table}[H]
\caption{Times registered for attack 1}
\begin{center}
...
\\
The results are presented for each key length, and we distinguish from optimal and worst case.


\begin{table}[H]
\caption{Times registered for attack 2, bit level}
\begin{center}
\begin{tabular}{l*{6}{c}r}
...
\begin{table}[H]
\caption{Times registered for attack 2, byte level}
\begin{center}
\begin{tabular}{l*{6}{c}r}
Key length        & Optimal case [ms] & Worst case [ms] \\
\hline
